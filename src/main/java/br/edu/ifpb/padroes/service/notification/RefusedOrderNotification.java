...
public class RefusedOrderNotification implements Listener {

    private String emailAdministration = "contact@food-store.com";

    @Override
    public void notifyEmail(String message) {
        System.out.println("send refused order mail notification to "+ emailAdministration);
    }

    @Override
    public void notifyEmail(String message, Customer customer) {
        System.out.println("send refused order mail notification to "+ customer.getEmail());
    }
}
